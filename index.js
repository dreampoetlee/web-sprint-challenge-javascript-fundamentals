...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
  /* Closure is a way to understand how different functions can see outside of themselves and into the global scope. 
  Which is why everything inside of the curly braces is enclosed and known as the closure. It's like a one way mirror.
  */
...
function animalNames(data) {
  ...
  data.forEach (i => displayNames.push(`name: ${i.animal_name}, scientific: ${i.scientific_name}`));
  return displayNames;
}
...
function lowerCaseNames(data) {
  let lowerAnimal = data.map(i => i['animal_name'].toLowerCase())
  return lowerAnimal
}
...
function lowPopulationAnimals(arr) {
  return arr.filter(i => i.population < 5);
}
